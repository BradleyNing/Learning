...
class GPAL(object):
    ...
    def GetResult(self, estimator, param_range=None):
        y_pred = estimator.predict(self.X_train)
        confmat = confusion_matrix(y_true=self.y_train, y_pred=y_pred)
        allnum = confmat.sum()
        ...
        metrics = {'precison': '%.4f' % precison,
                   'recall': '%.4f' % recall,
                   'FPR': '%.4f' % FPR,
                   'F1': '%.4f' % F1}
        y_pred = estimator.predict_proba(self.X_train)[:, 1]
        roc_auc = roc_auc_score(self.y_train, y_pred)
        auc = '%.4f' % roc_auc
        if param_range is None:
            result = {'voting classifier accuracy': accuracy,
                      'auc': auc,
                      'confusion_matrix': str(confmat),
                      'metrics': metrics}
        else:
            best_score = '%.4f' % estimator.best_score_
    ...
    def RunGridSearch(self):
        if self.algorithm == DAAS_DEF.alg_lr:
            ...
            result_lr = self.GetResult(gs_lr, param_range)
            result = {'train_file_name': self.train_file_name,
                      'X_train_shape': str(self.X_train.shape),
                      'result': result_lr,
                      'train_para': self.train_para}
            ...
            result_knn = self.GetResult(gs_knn, param_range)
            result = {'train_file_name': self.train_file_name,
                      'X_train_shape': str(self.X_train.shape),
                      'result': result_knn,
                      'train_para': self.train_para}
            ...
            result_rfc = self.GetResult(gs_rfc, param_grid)
            result = {'train_file_name': self.train_file_name,
                      'X_train_shape': str(self.X_train.shape),
                      'result': result_rfc,
                      'train_para': self.train_para}
    ...
    def RunCombiningModel(self):
        ...
        result_ens = self.GetResult(ensemble_voting)
    ...
    def Predict(self, req):
        ...
        temp_df_X = pd.DataFrame(self.X_train)
        temp_df_X.to_csv('./output/df_X.csv', index=False)
        temp_df_y = pd.DataFrame(self.y_train)
        temp_df_y.to_csv('./output/df_y.csv', index=False)
        temp_df_test = pd.DataFrame(self.X_test)
        temp_df_test.to_csv('./output/df_test.csv', index=False)
