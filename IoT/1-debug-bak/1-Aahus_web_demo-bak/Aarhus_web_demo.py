...
from Aarhus_admin_forms import Form_admin_get, Form_admin_put, Form_admin_show
...
def admin():
    ...
    if formPut.submitPut.data:
        if formPut.validate_on_submit():
            ...
            #auth=HTTPBasicAuth('Bradley-ZN00046', 'Bradley-ZN00046')
            res = requests.put(qUrl, headers=headers, json=req_body)
            res_info += 'status_code: '+str(res.status_code)+'\n'
            res = res.json()
    ...
    if formGet.submitRedLed.data:
        set_state = {"state": {
                      "desired": {"blinking":'red'}}}
        qttClient.publish('$aws/things/PI_Ireland/shadow/update', 
                       json.dumps(set_state), 1)

    if formGet.submitGreenLed.data:
        set_state = {"state": {
